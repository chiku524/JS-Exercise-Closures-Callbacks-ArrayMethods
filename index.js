...
function lowerCaseStrings(strings) {
  let arr = [];
  strings.forEach(string => arr.push(string.toLowerCase()));
  return arr;
}
...
function isItAnApple(strings) {
  return strings.map(element => element === 'apple' ? true : false);
}
...
function removeApple(strings) {
  return strings.filter(element => element != 'apple');
}
...
function getFullNames(runners) {
  return runners.map(object => object.last_name + ', ' + object.first_name);
}
...
function firstNamesAllCaps(runners) {
  return runners.map(object => object.first_name.toUpperCase());
}
